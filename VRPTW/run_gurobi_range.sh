...
Other options:
  ...
  --dataset-path PATH   Dataset path. Default: ../../classical_dataset_v1/vrptw/dataset/<split>/<CusN>
...
DATASET_PATH="${DATASET_PATH:-../../classical_dataset_v1/vrptw/dataset/${SPLIT}/${SCALE}}"
OUTPUT_PATH="${OUTPUT_PATH:-results/${SPLIT}/${SCALE}}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/gurobi_vrptw_${SCALE}_${SPLIT}_${START_INDEX}_${END_INDEX}_w${WORKERS}_t${THREADS}_${TIMESTAMP}.log}"
...
if [[ "$DETACH" == "1" && "${_GUROBI_DETACHED:-0}" != "1" ]]; then
  ...
  echo "Started detached VRPTW Gurobi run: PID=$!"
